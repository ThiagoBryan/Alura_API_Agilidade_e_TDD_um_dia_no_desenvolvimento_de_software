...
@SpringBootTest
@TestPropertySource(properties = { "DB_NAME=clines-test", "spring.jpa.hibernate.ddlAuto:create-drop" })
...
class UserControllerTest {
	...
	void shouldReturnAnUserById() throws Exception {
		var user = new User("Fulano", "fulano@teste.com", "123456");

		entityManager.persist(user);

		mockMvc.perform(get("/user/1")).andExpect(status().isOk()).andDo(log())
